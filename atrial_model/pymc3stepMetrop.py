...
# -*- coding: utf-8 -*-
"""
Created on Thu Nov 19 13:05:57 2020
...
class DEMetropolisJoint(PopulationArrayStepShared):
    
    name = "DEMetropolisJoint"
    ...
    **kwargs
        ...
        shared = pm.make_shared_replacements([], model)
        ...
        for i,var in enumerate(vars):
            ...
            if not lamb is None:
                lamb_sub = lamb[var.name]
            
            steps[var.name] = DEMetropolisSingle(
                var=var,
                S=S_sub,
...
#            self.stats_dtypes += steps[var.name].stats_dtypes
        ...
        sub_shared = {name:val for name, val in shared.items() if not name in vars}
        self.delta_logp = delta_logp(model.logpt, vars, sub_shared)
        super().__init__(vars, shared, blocked=blocked)
        
    def astep(self, q0):
        q = np.zeros_like(q0)
        for key, stepper in self.steps.items():
            stepper.parent = self
            slc = self.ordering[key][1]
            q[slc] = stepper.propose(q0[slc])
        
        dlogp = self.delta_logp(q, q0)
        
        q_new = np.zeros_like(q0)
        stats = {}
        for key, stepper in self.steps.items():
            slc = self.ordering[key][1]
            q_new[slc], [stat] = stepper.accept_step(q[slc], q0[slc])
            for stat_key in stat:
                if stat_key not in stats:
                    stats[stat_key] = []
                stats[stat_key].append(stat[stat_key])
        stats = {key: np.array(value) for key, value in stats.items()}
    ...
    def get_population(self, key):
        if self.population is None:
            return None
        return [pop[key] for pop in self.population]
    


class DEMetropolisSingle():
    """
    ...
    def __init__(
        ...
        **kwargs
        ...
        if shared is None:
            ...
            shared = {name:val for name, val in shared.items() if name != var}
...
#        pdb.set_trace()
        ...
        population = self.parent.get_population(self.varname)
        other_chains = self.parent.other_chains
        # differential evolution proposal
        # select two other chains
        ir1, ir2 = np.random.choice(other_chains, 2, replace=False)
        r1 = population[ir1]
        r2 = population[ir2]
        # propose a jump
